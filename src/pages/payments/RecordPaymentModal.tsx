...
export function RecordPaymentModal({
  ...
  const invoiceOptions = openInvoices.filter(
      ...
      <div className="flex flex-col gap-3.5">
          ...
          {(id) => (
            <Select
              ...
              options={invoiceOptions.map((i) => ({
                value: i.id,
                label: `${i.invoiceNo} · ${db.customers.find((c) => c.id === i.customerId)?.name} · ${inr(i.balance)} due`,
              }))}
            />
          )}
        </Field>
        {selected && (
          <p className="-mt-2 text-xs text-muted">
            Outstanding: <b className="text-ink">{inr(selected.balance)}</b>
          </p>
        )}
        ...
        <Field label="Reference" hint="UPI ref / cheque no. / transaction ID">
          {(id) => (
            <Input
              id={id}
              value={reference}

...
export function InvoicesListPage() {
  ...
  const rows = useMemo(() => {
    ...
    return views
      ...
      .filter((i) => {
        ...
        if (minAmount && i.total < Number(minAmount)) return false;
        if (balanceOnly && i.balance <= 0) return false;
        return true;
      })
      .sort((a, b) =>
  ...
  const tabCount = (key: TabKey) =>
    key === "all"
      ? views.length
      : views.filter((i) => i.derivedStatus === key).length;
  ...
  const byStatus = useMemo(() => {
    ...
    for (const i of views)
      m.set(i.derivedStatus, (m.get(i.derivedStatus) ?? 0) + 1);
    return [...m.entries()]
      .map(([name, value]) => ({
        name,
        value,
  ...
  const topCustomers = useMemo(() => {
    const m = new Map<string, number>();
    for (const i of views)
      m.set(i.customerId, (m.get(i.customerId) ?? 0) + i.total);
    const max = Math.max(...m.values(), 1);
    return [...m.entries()]
      .map(([id, value]) => ({
        customer: custById.get(id),
        value,
  ...
  const aging = useMemo(() => {
    ...
    return buckets.map((b) => {
      ...
      return {
        ...b,
        amount,
        pct: outstanding ? Math.round((amount / outstanding) * 100) : 0,
      };
    });
  }, [views, outstanding]);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 max-w-md">
          ...
          <p className="mt-1 text-[13px] text-muted">
            Create and manage sales invoices, track payments and monitor
            outstanding amounts.
          </p>
        </div>
        <Button
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            <Kpi
              ...
              label="Paid Amount"
              note={`${totalValue ? Math.round((paidAmount / totalValue) * 100) : 0}% of total`}
            />
            <Kpi
              icon={AlertTriangle}
              ...
              note={`${tabCount("Overdue")} invoices`}
            />
          </div>
          ...
          <div className="overflow-hidden rounded-card border border-line bg-white shadow-card">
            ...
            <div className="flex flex-wrap items-center gap-2.5 border-b border-line px-3 py-3">
              <Input
                ...
                placeholder="Search by invoice no., customer name, or reference…"
            ...
            >
              <FilterField label="Invoice value at least (₹)">
                <Input
                  type="number"
                  min={0}
                  value={minAmount}
              ...
              <FilterField label="Unsettled only">
                <Switch
                  checked={balanceOnly}
                  onChange={(v) => {
                    setBalanceOnly(v);
                  ...
                  label="Hide fully paid invoices"
                />
              </FilterField>
            </FilterPanel>
            ...
            {rows.length === 0 ? (
              <EmptyState
                ...
                description="Invoices generate from delivered orders."
                action={
                  <Button
                    size="sm"
                    variant="primary"
                        ...
                        >
                          ...
                          <td
                            className={cn(
                              ...
                              i.balance ? "text-orange-600" : "text-muted",
                            )}
                          >
                            {i.balance ? inr(i.balance) : "0"}
                          </td>
                          <td className="px-3 py-2.5 text-center">
                            <span
                              className={cn(
                                "rounded-full px-2 py-0.5 text-[11px] font-semibold whitespace-nowrap",
                                STATUS_TONE[i.derivedStatus] ??
                    ...
                    >
                      ...
                      <span className="tabular font-semibold text-ink">
                        ...
                        <span className="font-normal text-subtle">
                          (
                          {views.length
                            ? Math.round((s.value / views.length) * 100)
                            : 0}
                          %)
                        </span>
                      </span>
                ...
                {topCustomers.map((r, i) => (
                  <div key={r.customer!.id}>
                    ...
                    <div className="mt-1 h-2 overflow-hidden rounded-full bg-canvas">
                      <div
                        className="h-full rounded-full bg-fresh-500"
                ...
                <div key={b.label}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <p className="tabular text-[12.5px] font-semibold text-ink">
                      {inr(b.amount)}{" "}
                      <span className="font-normal text-muted">({b.pct}%)</span>
                    </p>
                  </div>
              ...
              {[
                {
                  ...
                  action: () => nav("/invoices/new"),
                },
                {
                  icon: Upload,
                  label: "Bulk Invoice Upload",
                  action: () => nav("/invoices/new"),
                },
                {
                  icon: Wallet,
      ...
      <div className="rounded-card bg-gradient-to-r from-brand-800 to-fresh-600 p-5 text-white">
        <p className="text-[15px] font-semibold">
          Healthy Finances. Stronger Partnerships.
        </p>
        <p className="text-[12.5px] text-white/80">
          Track your receivables, get paid faster and grow your business with
